...
export class ServerConfig {
  ...
  static readEnv(overrides?: Partial<ServerConfigValues>) {
    ...
    const didPlcUrl =
      process.env.CODESTASH_DID_PLC_URL || 'http://localhost:2582';
    const handleResolveNameservers = process.env
      .CODESTASH_HANDLE_RESOLVE_NAMESERVERS
      ? process.env.CODESTASH_HANDLE_RESOLVE_NAMESERVERS.split(',')
      : [];
    const cdnUrl =
      process.env.CODESTASH_CDN_URL || process.env.CODESTASH_IMG_URI_ENDPOINT;
    ...
    dataplaneUrls ??= process.env.CODESTASH_DATAPLANE_URLS
      ...
      : [];
    const dataplaneHttpVersion =
      process.env.CODESTASH_DATAPLANE_HTTP_VERSION || '2';
    ...
    const csyncIgnoreBadTls =
      process.env.CODESTASH_CSYNC_IGNORE_BAD_TLS === 'true';
    ...
    const courierHttpVersion =
      process.env.CODESTASH_COURIER_HTTP_VERSION || '2';
    ...
    const adminPasswords = envList(
      process.env.CODESTASH_ADMIN_PASSWORDS ||
        process.env.CODESTASH_ADMIN_PASSWORD,
    );
