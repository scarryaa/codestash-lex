...
import { MuteOperation, MuteOperation_Type } from '../src/proto/csync_pb';

describe('mutes', () => {
  let csync: CsyncService;
  let client: CsyncClient;

  beforeAll(async () => {
    csync = await CsyncService.create(
      envToCfg({
        dbUrl: process.env.DB_POSTGRES_URL,
        dbSchema: 'csync_mutes',
        apiKeys: ['key-1'],
        longPollTimeoutMs: 500,
      }),
    );
    await csync.ctx.db.migrateToLatestOrThrow();
    await csync.start();
    client = createClient({
      httpVersion: '1.1',
      baseUrl: `http://localhost:${csync.ctx.cfg.service.port}`,
  ...
  describe('addMuteOperation', () => {
    it('adds mute operations to add mutes.', async () => {
      ...
      await client.addMuteOperation({
        ...
        subject: 'at://did:example:d/org.codestash.graph.list/rkey1',
      });
      expect(await dumpMuteState(csync.ctx.db)).toEqual({
        'did:example:a': ['did:example:b', 'did:example:c'],
        'did:example:b': ['did:example:c'],
        'did:example:c': ['at://did:example:d/org.codestash.graph.list/rkey1'],
      });
    });
      ...
      for (let i = 0; i < 10; ++i) {
        ...
        for (let j = 0; j < 2; ++j) {
          await client.addMuteOperation({
            type: MuteOperation_Type.ADD,
            actorDid: `did:example:${i}`,
            subject: `at://did:example:0/org.codestash.graph.list/rkey${j}`,
          });
        }
      }
